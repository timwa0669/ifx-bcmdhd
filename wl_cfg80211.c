...
wl_dealloc_netinfo_by_wdev(struct bcm_cfg80211 *cfg, struct wireless_dev *wdev)
{
	...
	BCM_LIST_FOR_EACH_ENTRY_SAFE(_net_info, next, &cfg->net_list, list) {
		...
		if (wdev && (_net_info->wdev == wdev)) {
			...
			xr_role = _net_info->xr_role;
			if (xr_role == XR_MASTER)
				cfg->iface_xr_mst_cnt--;
			else if (xr_role == XR_SLAVE)
...
static s32 wl_cfg80211_tdls_mgmt(struct wiphy *wiphy, struct net_device *dev,
	...
	u16 status_code, u32 peer_capability, bool initiator, const u8 *buf,
	size_t len);
...
#endif // endif
	if ((cfg->xr_sta_mode == XR_STA_MODE_SINGLE) && cfg->xr_sta_enabled) {
		if ((dev == cfg->wdev->netdev) && cfg->xr_sta_active_wdev) {
			dhd_pub_t *xr_sta_pub = (dhd_pub_t *) dhd_get_pub(cfg->xr_sta_active_wdev->netdev);
	...
	if (chan_h->band == IEEE80211_BAND_5GHZ) {
		...
		if (unlikely(err)) {
			...
			err = wldev_iovar_setint_bsscfg(dev, "hogsqs",
				value, bssidx);
			WL_ERR(("set hogsqs cfg 0x%x, err %d\n", value, err));
		}
	}
#endif /* ENABLE_HOGSQS */
...
static bool wl_has_pmkid(u8 *parse, u32 len, u32 *offset_in_ie)
{
	...
	if (count) {
		if (offset_in_ie)
			*offset_in_ie = offset + WPA2_PMKID_COUNT_LEN;
		return true;
	}
	...
	struct cfg80211_update_owe_info *owe_info)
{
	...
	s32 bssidx = -1;
	int ret = BCME_OK;
	struct dhd_owe_info_buf *owe_info_buf = NULL;
	u8  *curr_ie_buf;
	char *ioctl_buf = NULL;
#ifdef WL_DHD_XR
	dhd_pub_t *dhdp = (dhd_pub_t *) dhd_get_pub(dev);
	...
	if (owe_info_buf->status_le16 != DOT11_SC_SUCCESS) {
		...
		if (owe_info_ie && DOT11_OWE_DH_PARAM_IE(owe_info_ie)) {
			...
			ie_setbuf = (ie_setbuf_t *)curr_ie_buf;

			/* Copy the vndr_ie SET command ("add"/"del") to the buffer */
			strncpy(ie_setbuf->cmd, "add", VNDR_IE_CMD_LEN - 1);
			ie_setbuf->cmd[VNDR_IE_CMD_LEN - 1] = '\0';
			del_add_ie_buf_len = VNDR_IE_CMD_LEN;

			/* Set the IE count - the buffer contains only 1 IE */
			ie_setbuf->ie_buffer.iecount = htod32(1);
			del_add_ie_buf_len += sizeof(int32);

			/* Set packet flag to indicate the appropriate frame will contain this IE */
			pktflag = htod32(VNDR_IE_ASSOCRSP_FLAG|VNDR_IE_CUSTOM_FLAG);
			memcpy((void *)&ie_setbuf->ie_buffer.ie_list[0].pktflag, &pktflag,
				sizeof(u32));
			del_add_ie_buf_len += sizeof(uint32);

			/* Set the IE Data */
			memcpy(&ie_setbuf->ie_buffer.ie_list[0].ie_data,
				owe_info_ie, owe_info_ie->len + TLV_HDR_LEN);
			del_add_ie_buf_len += (owe_info_ie->len + TLV_HDR_LEN);
		}
		...
		if (owe_info_ie) {
			...
			/* Update pmkid */
			if (wl_has_pmkid((u8 *)owe_info->ie,
					    owe_info->ie_len,
					    &pmkid_offset)) {
				int i;
				int npmkids = cfg->pmk_list->pmkids.count;
				...
				for (i = 0; i < npmkids; i++) {
					if (!memcmp(owe_info_buf->peer_mac,
							&cfg->pmk_list->pmkids.pmkid[i].bssid, ETH_ALEN)) {
						break;
					}
				}
				...
				if (i < WL_NUM_PMKIDS_MAX) {
					if (i == npmkids) {
						WL_ERR(("Not found in PMKID list %d\n", npmkids));
						ret = -EINVAL;
						goto end;
					}
					...
					WL_ERR(("Too many PMKSA entries cached %d\n", npmkids));
					ret = -EINVAL;
					goto end;
				}

				ret = wl_update_pmklist(dev, cfg->pmk_list, ret);
				if (ret < 0) {
					WL_ERR(("%s: set pmkid failed:%d\n", __FUNCTION__, ret));
					goto end;
				}

				owe_info_buf->with_pmkid = true;
				memcpy(owe_info_buf->pmkid, owe_info->ie + pmkid_offset,
						WLAN_PMKID_LEN);
			}
		}
		total_owe_info_len = sizeof(struct dhd_owe_info_buf) + del_add_ie_buf_len;

		ret = wldev_iovar_setbuf(dev, "owe_info", owe_info_buf, total_owe_info_len,
				ioctl_buf, WLC_IOCTL_SMLEN, &cfg->ioctl_buf_sync);

		if (unlikely(ret)) {
			WL_ERR(("could not set owe_info (%d)\n", ret));
		}
	}
end:
	if (ioctl_buf) {
