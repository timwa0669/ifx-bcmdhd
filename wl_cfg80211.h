...
#ifdef WL_DHD_DUAL_LOG
#define DUAL_CHIP1 1
#define DUAL_CHIP2 2
#ifdef WL_DHD_DUAL_CHIP1
#define DUAL_ROLE DUAL_CHIP1
#else
#define DUAL_ROLE DUAL_CHIP2
#endif /* WL_DHD_DUAL_CHIP1 */
#endif /* WL_DHD_DUAL_LOG */
...
do {	\
	if (wl_dbg_level & WL_DBG_ERR) {	\
		printk(KERN_INFO "%d "CFG80211_ERROR_TEXT "%s: ",DUAL_ROLE, __func__);	\
		pr_cont args;	\
		DHD_LOG_DUMP_WRITE("[%s] %s: ", dhd_log_dump_get_timestamp(), __func__);	\
		DHD_LOG_DUMP_WRITE args;	\
	}	\
...
do {	\
	if (wl_dbg_level & WL_DBG_ERR) {	\
		DHD_LOG_DUMP_WRITE("%d [%s] %s: ",DUAL_ROLE, dhd_log_dump_get_timestamp(), __func__);	\
		DHD_LOG_DUMP_WRITE args;	\
	}	\
} while (0)
#define	WL_INFORM_MEM(args)	\
do {	\
	if (wl_dbg_level & WL_DBG_INFO) {	\
		printk(KERN_INFO "%d "CFG80211_INFO_TEXT "%s : ",DUAL_ROLE, __func__);	\
		pr_cont args;	\
		DHD_LOG_DUMP_WRITE("%d [%s] %s: ",DUAL_ROLE, dhd_log_dump_get_timestamp(), __func__);	\
		DHD_LOG_DUMP_WRITE args;	\
	}	\
...
do {	\
	if (wl_dbg_level & WL_DBG_ERR) {	\
		printk(KERN_INFO CFG80211_ERROR_TEXT "%s : ", __func__);	\
		pr_cont args;	\
		DHD_LOG_DUMP_WRITE_EX("%d [%s] %s: ",DUAL_ROLE, dhd_log_dump_get_timestamp(), __func__);	\
		DHD_LOG_DUMP_WRITE_EX args;	\
	}	\
} while (0)
#define	WL_MEM(args)	\
...
#elif defined(WL_DHD_XR_LOG) /* !WL_DHD_XR_LOG && !WL_DHD_DUAL_LOG */
#define	WL_INFORM(args)									\
do {										\
	if (wl_dbg_level & WL_DBG_INFO) {				\
...
do {										\
	if (wl_dbg_level & WL_DBG_INFO) {				\
			printk(KERN_INFO "%d CFG80211-INFO) %s : ",DUAL_ROLE, __func__);	\
			pr_cont args;						\
		}								\
} while (0)
...
do {									\
	if (wl_dbg_level & WL_DBG_SCAN) {			\
		printk(KERN_INFO "%d CFG80211-SCAN) %s :",DUAL_ROLE, __func__);	\
		pr_cont args;							\
	}									\
} while (0)
#ifdef WL_TRACE
...
do {									\
	if (wl_dbg_level & WL_DBG_TRACE) {			\
		printk(KERN_INFO "%d CFG80211-TRACE) %s :",DUAL_ROLE, __func__);	\
		pr_cont args;							\
	}									\
} while (0)
#ifdef WL_TRACE_HW4
...
do {									\
	if (wl_dbg_level & WL_DBG_DBG) {			\
		printk(KERN_INFO "%d CFG80211-DEBUG) %s :",DUAL_ROLE, __func__);	\
		pr_cont args;							\
	}									\
} while (0)
#endif // endif
