	...
	struct wireless_dev *wdev, const void  *data, int len)
{
	...
	if (params->channels[0] > FREQ_START_6G_CHANNEL) {
		...
		/* In given channels,
		 * search 6g channel first and then select 5g and 2g
		 */
		int i = 0;
		int found = false;
		/* select higher band in channels given. */
		for(i = 0;i<params->chan_cnt; ++i) {
			if (params->channels[i] > FREQ_START_6G_CHANNEL) {
				found = true;
				break;
			}
		}
		...
		if (found) {
			...
			for(i = 0;i<params->chan_cnt; ++i) {
				if (params->channels[i] >= FREQ_START_5G_CHANNEL &&
					params->channels[i] <= FREQ_END_5G_CHANNEL) {
					found = true;
					break;
				}
			}
		...
		WL_INFORM_MEM(("ACS: selection band:%d from hw_mode=%d\n", params->band, params->hw_mode));
	}
#endif /* WL_6E */
...
wl_cfgvendor_acs_offload_post(struct wiphy *wiphy)
{
	...
	if (params->band == WLC_BAND_6G) {
		int start_6g_index = -1, num_6g_ch = 0;
		int select_6g_freq = 0;
		for (i = 0; i < params->chan_cnt; ++i) {
			if (params->channels[i] >= FREQ_START_6G_CHANNEL) {
				if (start_6g_index == -1)
					start_6g_index = i;
		...
		if (num_6g_ch > 0) {
			select_6g_freq = params->channels[start_6g_index + (RANDOM32() % num_6g_ch)];
			if ( select_6g_freq >= FREQ_START_6G_CHANNEL )
				selected = ieee80211_frequency_to_channel(select_6g_freq);
