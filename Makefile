...
DHDCFLAGS += -DWLFC_REUSESEQ
DHDCFLAGS += -DAMPDU_HOSTREORDER

# tput enhancement
# SDIO_F2_BLKSIZE is not expected to be changed by wlfc status,
# so disable DYNAMIC_F2_BLKSIZE.
#DHDCFLAGS += -DUSE_DYNAMIC_F2_BLKSIZE -DDYNAMIC_F2_BLKSIZE_FOR_NONLEGACY=128
# SDIO_F2_BLKSIZE should not exceed 256 for performance/compatibiltiy.
DHDCFLAGS += -DCUSTOM_SDIO_F2_BLKSIZE=128
# DHD_SDALIGN should adjust based on platform.
DHDCFLAGS += -DDHD_SDALIGN=32

# SDIO glom
DHDCFLAGS += -DBCMSDIOH_TXGLOM -DCUSTOM_GLOM_SETTING=8
# For performance reason, MAX_TXGLOM_SIZE should be set to 32, not 31 or 40.
DHDCFLAGS += -DCUSTOM_MAX_TXGLOM_SIZE=32
# SDIO ADMA2 should adjust based on platform. Disable it by default.
# Can be changed via DHDCFLAGS:CUSTOM_RXCHAIN or module_param:sd_txglom_mode.
DHDCFLAGS += -DCUSTOM_RXCHAIN=0

# TCPACK suppress
DHDCFLAGS += -DDHDTCPACK_SUPPRESS
# Expand maximum IPv4 TCPACK pool size
DHDCFLAGS += -DTCPACK_INFO_MAXNUM=40
# Expand maximum IPv4 TCPDATA pool size
DHDCFLAGS += -DTCPDATA_INFO_MAXNUM=40

DHDCFLAGS += -DRXFRAME_THREAD
# MAX_HDR_READ and FIRSTREAD should not exceed SDIO_F2_BLKSIZE.
