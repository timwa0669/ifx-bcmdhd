...
#if defined(WL_DHD_DUAL_LOG) && defined(DHD_LOG_DUMP)
...
do {	\
	if (dhd_msg_level & DHD_TRACE_VAL) { 	\
		printk(KERN_INFO "%d ", DUAL_ROLE);	\
		pr_cont args;	\
		DHD_LOG_DUMP_WRITE("%d [%s]: ", DUAL_ROLE, dhd_log_dump_get_timestamp());	\
		DHD_LOG_DUMP_WRITE args;	\
	}	\
} while (0)
#endif /* DHD_TRACE */
...
do {	\
	if (dhd_msg_level & DHD_EVENT_VAL) {	\
		...
		DHD_LOG_DUMP_WRITE_PRSRV("%d [%s]: ", DUAL_ROLE, dhd_log_dump_get_timestamp()); \
		DHD_LOG_DUMP_WRITE_PRSRV args;	\
	}	\
} while (0)
#endif /* DHD_PRSRV_MEM */
...
do {										\
	if (dhd_msg_level & DHD_ERROR_VAL) {    \
		printk(KERN_INFO "%d ", DUAL_ROLE);	\
		pr_cont args;	\
		DHD_LOG_DUMP_WRITE_EX("%d [%s]: ", DUAL_ROLE, dhd_log_dump_get_timestamp());	\
		DHD_LOG_DUMP_WRITE_EX args;	\
	}	\
} while (0)
#endif /* DHD_ERROR_EX */
...
#endif /* WL_DHD_DUAL_LOG && DHD_LOG_DUMP */
