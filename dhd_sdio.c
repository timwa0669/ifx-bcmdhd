...
dhdsdio_sr_init(dhd_bus_t *bus)
{
	...
	if (CHIPID(bus->sih->chip) == BCM43430_CHIP_ID ||
		...
		CHIPID(bus->sih->chip) == CYW55530_CHIP_ID ||
...
dhd_bus_rxctl(struct dhd_bus *bus, uchar *msg, uint msglen)
{
	...
	if (RXCTL_RESCHED_CNT > 0 && !rxlen && timeleft == 0) {
		cnt++;
                     ...
                     uint16 devid)
{
	...
	if (!DHD_NOPMU(bus)) {
		...
		if (!si_setcore(bus->sih, ARMCR4_CORE_ID, 0)) {
			...
			case CYW55530_CHIP_ID:
				if (bus->sih->chiprev == 0) {
					bus->dongle_ram_base = CR4_55500_RAM_BASE;
				}
				else {
					bus->dongle_ram_base = CR4_55500_A1_RAM_BASE;
				}
				break;
		...
		if (bus->sih->chip == CYW55500_CHIP_ID ||
			...
			if (bus->sih->chiprev == 0) {
				...
				/* H1 CP, Dedicated space for TRXV5 Header at RAMBASE only */
				bus->orig_ramsize -= (CR4_55500_A1_TCAM_SZ +
						CR4_55500_TRXV5_HDR_SZ);
			}
...
dhdsdio_download_code_file(struct dhd_bus *bus, char *pfw_path)
{
	...
	while ((len = dhd_os_get_image_block((char*)memptr, memblock_size, image))) {
		...
		if (trx_chk && (len >= sizeof(struct trx_header))) {
			...
			trx_hdr = (struct trx_header *)memptr;
			if (trx_hdr->magic == TRX_MAGIC) {
				DHD_INFO(("trx_hdr->magic (%d) trx_hdr->flag_version (%d)\n",
						trx_hdr->magic, trx_hdr->flag_version >> 16));
				/* CYW555x0, we need to write TRX header at RAMSTART */
				if ((trx_hdr->flag_version >> 16)  == TRX_VERSION5) {
					offset -= sizeof(struct trxv5_header);
				}
				else
